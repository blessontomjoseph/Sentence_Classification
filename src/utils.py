...
class MDWDataset:
    ...
    def __getitem__(self,items):
        ...
        batch = self.tokenizer(text=feature_1,text_pair=feature_2,truncation=True,padding='max_length',max_length=200,return_tensors='pt')
...
class Engine:
    ...
    def train(self,data_loader):
        ...
        for batch in tqdm(data_loader):
            ...
            preds_full=np.append(preds_full,preds.to('cpu').numpy(),axis=-1)
            targets_full=np.append(targets_full,targets.to('cpu').numpy(),axis=-1)
...
class Model(nn.Module):
    def __init__(self,model,dropout,linear_input_size):
        super().__init__()
        self.model=model
        self.dropout=dropout
        self.linear_input_size=linear_input_size
        self.out=nn.Sequential( nn.Dropout(p=self.dropout),
                                nn.Linear(self.linear_input_size,3)
                              )
